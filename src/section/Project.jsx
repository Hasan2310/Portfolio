...
const projects = [
  {
    ...
    link: "https://tfly-production.up.railway.app/",
    stack: [
...
const Project = () => {
  ...
  const [clicked, setClicked] = useState(false); // untuk toggle warna di mobile
  const project = projects[page];

  const paginate = (newPage) => {
    setPage(([prevPage]) => [newPage, newPage > prevPage ? 1 : -1]);
    setClicked(false); // reset klik saat ganti project
  };
    ...
    <section id="project" className="relative h-auto w-full px-6 md:px-30 py-10">
      ...
      <AnimatePresence custom={direction} mode="wait">
        <motion.div
          ...
          animate="center"
          exit="exit"
          className={`flex flex-col md:flex-row items-center gap-6 md:gap-10 text-center md:text-left ${page % 2 === 0 ? "md:flex-row-reverse" : ""}`}
        >
          {/* Gambar */}
          <img
            src={project.img}
            alt={project.title}
            className={`${project.imgSize} h-auto rounded-xl shadow-lg
              filter grayscale md:hover:grayscale-0 transition duration-500 cursor-pointer
          ...
          <div className="max-w-4xl">
            <h1 className="text-3xl sm:text-5xl md:text-7xl font-bold text-anton mb-4">{project.title}</h1>
            <p className="text-base text-justify md:text-xl text-gray-300 mb-6">{project.desc}</p>
      ...
      <div className="absolute right-3 md:right-6 top-1/2 -translate-y-1/2 flex flex-col gap-3">
        {projects.map((_, idx) => (
          <span
            ...
            className={`w-3 h-3 rounded-full cursor-pointer transition-all ${idx === page ? "bg-purple-gradient scale-125" : "bg-gray-500"}`}
          />
        ))}
      </div>
    </section>
